...
class TD3:
    ...
    def choose_action(self, observation, train=True):
        ...
        # actions_tensor_grad.retain_grad()
        ...
        # all_act_sm_log = F.log_softmax(y_grad)
        ...
        if 'LC' in self.name_RL:
            action_emseble = action.squeeze().detach().cpu().numpy() - 1 * act_bias
        else:
    ...
    def learn_class(self, X, Y, val_x, val_y):
        ...
        for iter_count in range(400):
            ...
            fpr, tpr, thresholds = roc_curve(Y.loc[index_raw], pred_y_0_raw)
        pass

    def learn(self):
        states, actions, rewards, states_, terminals = self.memory.sample_buffer()
        reward_pred = self.predict_y(states, actions)
        rewards[rewards == 1] = 1.5
        rewards[rewards == 0] = -1
        rewards += 0.5 * (reward_pred[:, 0] * 2 - 1)
        ...
        self.classifier.eval()
        actions_tensor_grad = self.target_actor.forward(next_states_tensor)
        actions_tensor_grad.retain_grad()
        ...
        actor_loss = 1e-1 * loss_1 * lmbda1 + 2 * loss_2
